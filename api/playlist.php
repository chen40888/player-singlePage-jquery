...
function playlist_item($id) {
	...
	switch($_SERVER['REQUEST_METHOD']) {
		case 'GET':
			...
			if(!empty($data['songs'])) $data['songs'] = json_decode($data['songs'],TRUE);
		...
		case 'GET':
			...
			if($stmt->execute()){
				...
				while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
					$data[] = $row;
					if(!empty($data[$index]['songs'])) $data[$index]['songs'] = json_decode($data[$index]['songs'], true);
					$index++;
				}
			}
			//Log::w($data, '$data');
